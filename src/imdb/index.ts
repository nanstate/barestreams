...
const DATASETS: DatasetDescriptor[] = [
	{ gz: "title.basics.tsv.gz", tsv: "title.basics.tsv" },
];
...
const findLineByTconst = async (
	...
	try {
		...
		while (low <= high) {
			...
			const { line, start, end } = await readLineAtOffset(
				handle,
				mid,
				size,
			);
...
export const getTitleBasics = async (
	tconst: string,
): Promise<TitleBasics | null> => {
