...
const resolveQuality = (options: StreamDisplayOptions): string => {
	const hint =
		extractQualityHint(options.torrentName ?? "") ??
		extractQualityHint(options.quality ?? "") ??
		null;
	return hint ?? "480p";
};
...
export const formatStreamDisplay = (
	...
	const slugLine =
		buildTorrentSlug(options.torrentName, imdbTitle) ||
	...
	const lines = [imdbTitle, episodeLine, slugDisplay, infoLine].filter(
